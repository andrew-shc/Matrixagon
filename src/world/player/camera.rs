use crate::datatype::{Rotation, Dimension, CamDirection, Position, Line, ChunkUnit, BlockUnit};
use crate::world::chunk::{Chunk, CHUNK_SIZE};
use crate::world::player::EDIT_RADIUS;
use crate::world::block::Block;
use crate::world::block::state::Matter;

use vulkano::image::Extent::E1D;
    ...
    pub fn raycast_break(&self, chunks: &Vec<Chunk>) -> Option<(Position<BlockUnit>, Block)> {
        ...
        'ray: while block_pos == None {
            ...
            for chunk in chunks.iter() {
                let bx = chunk.position.x.into_block().0;
                let bX = (chunk.position.x.incr()).into_block().0;
                let by = chunk.position.y.into_block().0;
                let bY = (chunk.position.y.incr()).into_block().0;
                let bz = chunk.position.z.into_block().0;
                let bZ = (chunk.position.z.incr()).into_block().0;
